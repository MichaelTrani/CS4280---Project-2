...
static const char* tokenName[] = {  // token to text for error messages
	[BEGIN_KYD] = "begin_tk",[END_KYD] = "end_tk",[LOOP_KYD] = "loop_tk",[VOID_KYD] = "void_tk",[VAR_KYD] = "car_tk",[RETURN_KYD] = "return_tk",[IN_KYD] = "in_tk",[OUT_KYD] = "out_tk",
...
treeNode* stat(int depth){
	...
	else if (tk.tkType == OUT_KYD) {
		temp->c1 = out(depth);
		//period
	}

	else if (tk.tkType == BEGIN_KYD) {
		temp->c1 = block(depth);
		return temp;
		// no period
	}

	else if (tk.tkType == IFFY_KYD) {
		temp->c1 = if_f(depth);
		//period
	}

	else if (tk.tkType == LOOP_KYD) {
		temp->c1 = loop(depth);
		//period
	}
